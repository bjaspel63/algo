...
{title:"Mission 2",start:[0,0],goal:[2,3],walls:[[0,2],[1,1],[2,1],[3,3],[4,0]],solution:["R","F","L","F","F","R","F"]},
{title:"Mission 3",start:[0,0],goal:[3,3],walls:[[0,3],[1,0],[1,2],[2,2],[3,1],[4,4]],solution:["F","L","F","R","F","F","R","F"]},
{title:"Mission 4",start:[0,0],goal:[4,3],walls:[[0,2],[1,1],[2,1],[2,4],[3,2],[4,1]],solution:["R","F","L","F","F","R","F","F"]},
{title:"Mission 5",start:[0,0],goal:[4,4],walls:[[0,3],[1,0],[1,2],[2,2],[3,1],[3,4]],solution:["F","L","F","R","F","F","R","F","F"]}
];
...
async function teamSubmit(points,meta){
 ...
 const result=await runTransaction(teamRef,current=>{
  ...
  if(index<max-1){current.progress.index=index+1;current.progress.questionStartedAt=Date.now()}
...
function renderMCQ(i,p){
 ...
 clock=setInterval(async()=>{
  ...
  if(left<=0 || levelElapsed>=300000){clearInterval(clock);await teamSubmit(0,"timeout")}
 },500);
